...
static void lv_opengles_index_buffer_init(const unsigned short * data, unsigned int count);
...
static bool vao_supported = false;
...
static void lv_opengles_vertex_array_init(void)
{
    vao_supported = (glGenVertexArraysOES != NULL);
    if(vao_supported) {
        GL_CALL(glGenVertexArraysOES(1, &vertex_array_id));
    }
}
...
static void lv_opengles_vertex_array_deinit(void)
{
    ...
    if(vao_supported) {
        GL_CALL(glDeleteVertexArraysOES(1, &vertex_array_id));
    }
    vertex_array_id = 0;
}
...
{
    if(vao_supported) {
        GL_CALL(glBindVertexArrayOES(vertex_array_id));
    }
}
...
static void lv_opengles_index_buffer_init(const unsigned short * data, unsigned int count)
{
    ...
    GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * sizeof(GLushort), data, GL_STATIC_DRAW));
}
...
static unsigned int lv_opengles_shader_manager_init(void)
{
    for(lv_opengl_glsl_version_t version = LV_OPENGL_GLSL_VERSION_100; version < LV_OPENGL_GLSL_VERSION_LAST; ++version) {
...
static void lv_opengles_render_draw(void)
{
    ...
    GL_CALL(glDrawElements(GL_TRIANGLES, count, GL_UNSIGNED_SHORT, NULL));
    LV_PROFILER_DRAW_END;
}
